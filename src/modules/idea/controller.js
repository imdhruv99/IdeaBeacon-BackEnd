...
export const createIdea = async (req, res) => {
    try {
        ...
        res.status(HttpStatusCodes.CREATED.code).json({ status: true, message: responseStrings.createIdeaSuccessMessage, data: createdIdea });
    } catch (error) {
        logger.error(`Error creating idea: ${error.message}`);
        res.status(HttpStatusCodes.INTERNAL_SERVER_ERROR.code).json({ status: false, message: responseStrings.createIdeaErrorMessage });
    }
};
